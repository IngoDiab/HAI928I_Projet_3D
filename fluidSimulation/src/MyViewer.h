...
public :
    ...
    void draw() {
        ...
        for( unsigned int t = 0 ; t < mesh.triangles.size() ; ++t ) {
            QVector3D const & p0 = mesh.vertices[ mesh.triangles[t][0] ].p;
            QVector3D const & p1 = mesh.vertices[ mesh.triangles[t][1] ].p;
            QVector3D const & p2 = mesh.vertices[ mesh.triangles[t][2] ].p;
            QVector3D const & n = QVector3D::crossProduct( p1-p0 , p2-p0 ).normalized();
...
//                QVector3D bb(FLT_MAX,FLT_MAX,FLT_MAX) , BB(-FLT_MAX,-FLT_MAX,-FLT_MAX);
//                for( unsigned int v = 0 ; v < mesh.vertices.size() ; ++v ) {
//                    bb = QVector3D::min(bb , mesh.vertices[v]);
//                    BB = QVector3D::max(BB , mesh.vertices[v]);
//                }
//                adjustCamera(bb,BB);
//                update();
//            }
